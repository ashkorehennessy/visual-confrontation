...
def auto_pilot():
    ...
    spd_pid = PID(Kp=0.5, Kd=20, outmax=250, outmin=-250)
    robot = robotPi()
    ...
    while time.time() - start_time < 0.7:
        ...
        if diff > 300:
            print("left")
            robot.movement.left_ward(speed=0, turn=400, times=400)
        elif diff < -300:
            print("right")
            robot.movement.right_ward(speed=0, turn=-400, times=50)
        else:    
            print("forward")
    ...
    while now_time - start_time < 4.5: 
        ...
        if diff > 150:
            print("left")
            robot.movement.left_ward(angle=0,speed=150, turn=400, times=200)
        elif diff < -150:
            print("right")
            robot.movement.right_ward(angle=0,speed=150, turn=-400, times=200)
        else:
    ...
    while now_time - start_time < 6.9: 
        ...
        if diff > 300:
            print("left")
            robot.movement.left_ward(angle=0,speed=150, turn=450, times=200)
        elif diff < -300:
            print("right")
            robot.movement.left_ward(angle=0,speed=150, turn=-220, times=200)
        else:
            robot.movement.move_forward(speed=150, times=200)
            print("forward")
        now_time = time.time()
    ...
    while False:
        ...
        if left_count == 0:
            left_ratio = 0
            if adj is False:
                robot.movement.right_ward(angle=270,speed=20, turn=0, times=50)

        else :
            left_ratio = 3600 / left_count
            if left_ratio < 6:
                robot.movement.left_ward(angle=80,speed=40, turn=-42, times=80)
                adj = True
        # 输出信息
