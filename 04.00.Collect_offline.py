...
class CollectTrainingData(object):
    ...
    input:
        commands and video
    output:
        带有标签的灰度图像集，标签（0, 1, 2, 3 , 4, 5, 6）分别代表（前进， 左转，右转，停止,前进，左转，右转）
        ...
        while cap.isOpened():
            ...
            # road banner front
                ...
                if num_list[4] < self.range:
                    ...
                    print("Banner front image collect: ", num_list[4])
            ...
            # road banner left
                ...
                if num_list[5] < self.range:
                    num_list[5] += 1
                    total_images_collected += 1
                    self.mv.stop()
                    res = np.reshape(res, [1, -1])
                    images = np.vstack((images, res))
                    labels = np.vstack((labels, self.k[5]))
                    print("Banner left image collect: ", num_list[5])
            ...
            # road banner right
                ...
                if num_list[6] < self.range:
                    num_list[6] += 1
                    total_images_collected += 1
                    self.mv.stop()
                    res = np.reshape(res, [1, -1])
                    images = np.vstack((images, res))
                    labels = np.vstack((labels, self.k[6]))
                    print("Banner right image collect: ", num_list[6])
