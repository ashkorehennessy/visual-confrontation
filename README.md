...
安装好termux或MT管理器或juicessh等可以打开本地终端的软件。

将树莓派和手机用数据线连起来，手机设置里打开USB网络共享，此时手机会创建一个网络接口，名字一般为```rndis0```，进入手机上的终端软件，输入命令
...
ssh可以算是性能开销最小的连接方式了，只有一个命令行界面，可以尽量腾出cpu处理图形。
...
自行配置好国内apt软件源，推荐中科大的(ustc)，输入以下命令安装xrdp
